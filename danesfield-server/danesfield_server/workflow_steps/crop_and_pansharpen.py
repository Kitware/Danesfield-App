...
class CropAndPansharpenStep(DanesfieldWorkflowStep):
    ...
    def __init__(self):
        super(CropAndPansharpenStep, self).__init__(DanesfieldStep.CROP_AND_PANSHARPEN)
    ...
    def run(self, jobInfo, outputFolder):
        ...
        msiImageFiles = self.getFiles(initWorkingSet, isMsiImage)

        # Get PAN source image files
        panImageFiles = self.getFiles(initWorkingSet, isPanImage)
